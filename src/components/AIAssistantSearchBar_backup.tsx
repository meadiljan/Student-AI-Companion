...
// All task management is now handled by AI LLM parsing instead of rule-based functions
...
export default function AIAssistantSearchBar(props: AIAssistantSearchBarProps = {}) {
  ...
  const callAIModelForAgent = async (userQuery: string) => {
    try {
      ...
      const taskManagementPrompt = `You are an intelligent task management assistant. Your job is to understand user requests and determine what task management action to take.
...
CURRENT TASKS:
${tasks.map(task => `- ID: ${task.id}, Title: "${task.title}", Status: ${task.status}, Priority: ${task.priority}, Due: ${task.dueDate}${task.dueTime ? ` at ${task.dueTime}` : ''}, Course: ${task.course}`).join('\n')}
...
Please respond in this exact JSON format:
{
  ...
  "taskData": {
    // For CREATE_TASK: Include title, description, dueDate, dueTime, priority, course
    // For UPDATE_TASK: Include taskId and fields to update
    // For DELETE_TASK/COMPLETE_TASK/STAR_TASK: Include taskId
    // For CLARIFY: Include "clarificationQuestion"
  },
  "response": "Friendly response to show the user what you're doing"
}

Handle typos, grammar issues, and unclear requests gracefully. If uncertain, ask for clarification. Be helpful and understanding.`;
    ...
    try {
      ...
      try {
        ...
        setChatMessages(prev => [
          ...
          { 
            role: 'assistant', 
            content: "I understand you want to manage tasks, but I had trouble parsing your request. Could you please rephrase it? For example:\n- 'Create task: Study for exam tomorrow at 2pm'\n- 'Complete task: Typography Research Paper'\n- 'Delete task with title Assignment 1'" 
          }
        ]);
        return;
      }
      ...
      switch (action) {
        case 'CREATE_TASK':
          if (taskData && taskData.title) {
            ...
            setTimeout(() => {
              setChatMessages(prev => [
                ...prev,
                { role: 'assistant', content: `✅ Task "${newTask.title}" created successfully!` }
              ]);
              toast({
                title: "Task Created! 🎉",
        ...
        case 'UPDATE_TASK':
          if (taskData && taskData.taskId) {
            ...
            const updatedTask = tasks.find(t => t.id === taskData.taskId);
            setTimeout(() => {
              setChatMessages(prev => [
                ...prev,
        ...
        case 'DELETE_TASK':
          if (taskData && taskData.taskId) {
            const taskToDelete = tasks.find(t => t.id === taskData.taskId);
        ...
        case 'COMPLETE_TASK':
          if (taskData && taskData.taskId) {
            toggleCompleted(taskData.taskId);
            const completedTask = tasks.find(t => t.id === taskData.taskId);
            setTimeout(() => {
              setChatMessages(prev => [
                ...prev,
        ...
        case 'STAR_TASK':
          if (taskData && taskData.taskId) {
            toggleStarred(taskData.taskId);
            const starredTask = tasks.find(t => t.id === taskData.taskId);
            setTimeout(() => {
              setChatMessages(prev => [
                ...prev,
        ...
        case 'CLARIFY':
          // AI is asking for clarification - response already shown above
          break;
      ...
      setChatMessages(prev => [
        ...
        { 
          role: 'assistant', 
          content: "I encountered an error while processing your request. Please try again with a clearer task management request." 
        }
      ]);
    }
  };
